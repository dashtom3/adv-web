<template>
  <div>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true">
        <el-form-item>
          <el-button type="primary" @click="addAdv">添加资源</el-button>
        </el-form-item>
      </el-form>
    </el-col>
  ...
  export default {
      ...
      return {
        ...
        addProjectInfo: {
          ...
          content: null,
				...
				types: ['不限', '卡券', '会员资源', '线下网点', '微信公众号', '线上流量资源', '实体产品', '线下场地', '媒体广告', '其他'],
        ...
        qiNiuToken: null,
        rules: {
          name: [{ required: 'true', message: '请输入资源名字', trigger: 'blur' }],
    ...
    methods: {
      ...
      addProjectClick () {
        this.$refs['addProjectInfo'].validate((valid) => {
          if (valid) {
            ...
            global.axiosPostReq('resource/add', this.addProjectInfo)
            .then((res) => {
              if (res.data.callStatus === 'SUCCEED') {
      ...
      editChildren (index) {
        ...
        this.addProjectInfo.resourceId = this.projectLists[index].id
      ...
      editProjectClick () {
        this.$refs['addProjectInfo'].validate((valid) => {
          if (valid) {
            global.axiosPostReq('resource/update', this.addProjectInfo)
            .then((res) => {
              if (res.data.callStatus === 'SUCCEED') {
      ...
      handleDelete (id) {
        ...
        this.$confirm('是否删除?', '提示', {
          ...
          global.axiosPostReq('resource/delete', adverMsg)
          .then((res) => {
            if (res.data.callStatus === 'SUCCEED') {
              self.$message({
                type: 'success',
